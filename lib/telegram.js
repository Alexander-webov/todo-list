...
export async function sendNewProjectNotifications(newProjects) {
  ...
  const { data: allProfiles } = await db
    .from('profiles')
    .select('telegram_chat_id, premium_until, filter_categories, filter_sources, filter_keywords')
    .eq('is_premium', true)
    .not('telegram_chat_id', 'is', null);

  if (!allProfiles?.length) return;

  // Только активные премиум-пользователи
  const activeUsers = allProfiles.filter(u =>
