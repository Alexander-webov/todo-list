...
const MIN_BUDGET_INT = 50;

// Максимум постов за один cron-запуск (защита от спама)
...
function selectBestProjects(projects, maxCount) {
  // Скорим и сортируем
  const scored = projects
    .map(p => ({ ...p, _score: scoreProject(p) }))
    .filter(p => p._score > 0) // убираем спам и совсем плохие
    .sort((a, b) => b._score - a._score);

  // Разнообразие категорий: не больше 3 подряд одной категории
  const selected = [];
  const catCount = {};
  ...
  for (const p of scored) {
    ...
    if (catCount[cat] > 3) continue; // пропускаем если уже 3 такой категории
    selected.push(p);
  }
...
// ─── ФОРМАТИРОВАНИЕ ПОСТА (новый формат) ───
...
async function getTodayProjectCount() {
  const db = supabaseAdmin();
...
export async function sendNewProjectNotifications(newProjects) {
  ...
  // === 1. ПУБЛИЧНЫЕ КАНАЛЫ — только лучшие заказы ===
  const hasChannels = CHANNEL_RU || CHANNEL_INT;
  if (hasChannels) {
    const paused = await isPinnedAdActive();

    if (!paused) {
      const ruProjects = newProjects.filter(p => RU_SOURCE_KEYS.includes(p.source));
      const intProjects = newProjects.filter(p => !RU_SOURCE_KEYS.includes(p.source));

      // Отбираем лучшие
      const bestRu = selectBestProjects(ruProjects, MAX_POSTS_PER_RUN);
      const bestInt = selectBestProjects(intProjects, MAX_POSTS_PER_RUN);

      if (CHANNEL_RU && bestRu.length > 0) {
        for (const project of bestRu) {
          const count = await getAndIncrementPostCount();
          if (count % AD_EVERY_N_POSTS === 0) {
            await postFreeAdToChannel(CHANNEL_RU);
            await new Promise(r => setTimeout(r, 500));
          }
          await sendTelegramMessage(CHANNEL_RU, formatChannelPost(project, totalToday));
          await new Promise(r => setTimeout(r, 500));
        }
        console.log(`[Telegram RU] Лучших: ${bestRu.length} из ${ruProjects.length}`);
      }

      if (CHANNEL_INT && bestInt.length > 0) {
        for (const project of bestInt) {
          const count = await getAndIncrementPostCount();
          if (count % AD_EVERY_N_POSTS === 0) {
            await postFreeAdToChannel(CHANNEL_INT);
            await new Promise(r => setTimeout(r, 500));
          }
          await sendTelegramMessage(CHANNEL_INT, formatChannelPost(project, totalToday));
          await new Promise(r => setTimeout(r, 500));
        }
        console.log(`[Telegram INT] Лучших: ${bestInt.length} из ${intProjects.length}`);
      }
    }
  }

  // === 2. ЛИЧНЫЕ УВЕДОМЛЕНИЯ — всё как раньше ===
