  ...
  for (const feedUrl of UPWORK_RSS_FEEDS) {
    try {
      ...
      for (const item of feed.items || []) {
        ...
        const category = item.categories?.[0] || extractCategory(item.title + ' ' + content);
...
function extractCategory(text = '') {
  const cats = {
    'Web Development': /web|html|css|react|vue|angular|next\.?js|node|wordpress|shopify/i,
    'Mobile': /mobile|ios|android|flutter|react\s*native|swift|kotlin/i,
    'Design': /design|figma|sketch|photoshop|logo|ui[\s/]ux|graphic/i,
    'Writing': /writ|copy|content|blog|article|ghostwrit/i,
    'Marketing': /market|seo|social\s*media|ads|ppc/i,
    'Data': /data|python|ml|machine\s*learn|ai|analytics/i,
    'Backend': /backend|api|server|database|sql|postgresql|mongodb|php|laravel/i,
    'DevOps': /devops|docker|kubernetes|aws|azure|ci[\s/]cd/i,
  };
