...
...(fl.status          === 'fulfilled' ? fl.value          : []),
  ...
  if (customersToUpsert.length > 0) {
    ...
    const { error: custError } = await db.from('customers')
      .upsert(uniqueCustomers, { onConflict: 'external_id,source', ignoreDuplicates: false });
    if (custError) console.error('[Customers] Ошибка upsert:', custError.message);
    else console.log(`[Customers] Обновлено заказчиков: ${uniqueCustomers.length}`);
  }
